...
class Prot_Hop:
    ...
    def setting_properties(self):
        ...
        self.t = np.arange(self.n_max+1)*self.dt
        self.find_O_i(0)
    ...
    def viscosity(self, plotting=False, cubicspline=False, padding=0):
        ...
        for i in range(len(visc)):
            ...
            if plotting is True:
                j = re.findall(r'\d+', self.folder)[0]
                string = ' run ' + j
                plt.figure('pressure autocorrelation')
                plt.plot(t, acorr, label=string)

            integral = sp.integrate.simpson(acorr, x=t, even='first')
    ...
    def diffusion(self, MSD_in, N_specie, t=False, m=False, plotting=False):
        ...
        Minc = 20  # minimum number of points included in the fit
        Mmax = 50  # maximum number of points included in the fit
    ...
    def pressure(self, plotting=False, filter_width=0, skip=0):
        """
        ...
        if plotting is True:
            ...
            plt.figure('pressure')
            plt.plot(self.t, press, label=string)
    ...
    def kin_energy(self, plotting=False, filter_width=0, skip=0):
        """
        ...
        if plotting is True:
            ...
            plt.plot(self.t, ener, label=string)

        return energy

    def pot_energy(self, plotting=False, filter_width=0, skip=0):
        """
    ...
    def tot_energy(self, plotting=False, filter_width=0, skip=0):
        """
    ...
    def temperature(self, plotting=False, filter_width=0, skip=0):
        """
        ...
        if plotting is True:
            ...
            plt.plot(self.t, temp, label=string)
